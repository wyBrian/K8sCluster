...
function generate_tracker_addresses () {
    TRACKER_NUM=${TRACKER_NUM:-1}
    POD_NAME=${POD_NAME:-"tracker"}
    SVC_NAME=${SVC_NAME:-"tracker"}
    NAMESPACE=${NAMESPACE:-"default"}
    TRACKERS=`expr $TRACKER_NUM - 1`
    ALL_TRACKERS=""
    for i in `seq 0 $TRACKERS`
        do
            NEW_TRACKER="tracker_server=${POD_NAME}-$i.${SVC_NAME}.${NAMESPACE}.svc.cluster.local:22122"
...
function fdfs_set () {
    if [ "$1" = "monitor" ] ; then
        if [ -n "$TRACKER_NUM" ] ; then  
          sed -i "s|tracker_server=.*$|${ALL_TRACKERS}|g" /etc/fdfs/client.conf
        fi
    ...
    if [ -n "$TRACKER_NUM" ] ; then  
        sed -i "s|tracker_server=.*$|${ALL_TRACKERS}|g" /etc/fdfs/storage.conf
        sed -i "s|tracker_server=.*$|${ALL_TRACKERS}|g" /etc/fdfs/client.conf
        sed -i "s|tracker_server=.*$|${ALL_TRACKERS}|g" /etc/fdfs/mod_fastdfs.conf
    fi
    
    sed -i "s|group_name=.*$|group_name=${GROUP_NAME}|g" /etc/fdfs/storage.conf
    sed -i "s|group_name=group0$|group_name=${GROUP_NAME}|g" /etc/fdfs/mod_fastdfs.conf
